...
public class InputSystem : MonoBehaviour
{
    ...
    public List<PlayerInputInfo> GetInputResult(List<PlayerInputInfo> playerInput) //判斷是否在輸入成功區間
    {
        ...
        //Debug.Log("suscess" + timeResult.Count);
        for (int i = 0; i < timeResult.Count; i++)
        {   //區間判斷
            int j = 0;
            ...
            if (i == timeResult.Count - 1)
            {  //後面沒有資料
                result.Add(timeResult[i]); //輸出result
                break; //跳出迴圈
            }
            else
            {   //有資料判斷是否是EInputCondition.Two or Four
    ...
    EColor colorResult;
    EColor InputColorResult(EColor color1, EColor color2)
    {//回傳顏色結果
        switch (color1)
        { //判斷顏色組合
            case EColor.RED:
